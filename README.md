...
- `sources.yaml` lists the exact 22 active sources and the separate compatibility-only predecessor
...
children are the 22 repositories. The lock command requires every checkout to have a clean `main`
...
compare the committed lock with the current 22 remote `main` heads; moving-tip freshness is kept
