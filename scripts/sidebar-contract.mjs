...
export const documentationFamilies = familyTaxonomy.families.map((family) => ({
  ...
  expectedMembers: family.expectedMembers,
...
export function assertDocumentationFamilyDistribution(sourceManifests) {
  ...
  for (const family of documentationFamilies) {
    if (counts[family.id] !== family.expectedMembers) {
      throw new Error(`${family.id} documentation family has ${counts[family.id]} repositories; expected ${family.expectedMembers}`);
    }
  }
  return counts;
}
