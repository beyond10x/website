...
const websiteCommit = resolveWebsiteCommit({bootstrap, lockBytes});
...
function resolveWebsiteCommit({bootstrap, lockBytes}) {
  const candidate = process.env.GITHUB_SHA ?? gitCommit();
  if (/^[0-9a-f]{40}$/.test(candidate ?? '') && !/^0+$/.test(candidate)) return candidate;
