...
export function validateSourceLockDocument(roster, lock, {allowBootstrap = false} = {}) {
  ...
  if (expected.length === 0 || new Set(expected).size !== expected.length || expected.join('\n') !== [...expected].sort(compareUtf8).join('\n')) {
    throw new Error('sources.yaml must contain a non-empty, unique, sorted source-repository roster');
  }
  ...
  if (actual.length !== expected.length || actual.join('\n') !== expected.join('\n')) {
    throw new Error(`production sources.lock.json must contain the exact sorted ${expected.length}-repository roster`);
  }
