...
- `getting-started` is predecessor history, not an authority or a source dependency. It remains an
  explicit compatibility-only façade; do not add it to the 21-source lock.
...
21 source commits. The artifact crawler must cover manifest URLs, effective redirect targets, alias
