...
test('rendered Markdown removes complete managed documentation ranges without mutating source bytes', () => {
  const source = [
    ...
    '| Template | Path |',
    '| --- | --- |',
    '| Experiment | exp-tdd-<task>.json and `fixtures/<task>/` |',
    '',
    '<details><summary>Authored HTML</summary>Kept intact.</details>',
    '',
    '## Kept {#kept}',
  ].join('\n');
  ...
  assert.match(rendered, /exp-tdd-&lt;task&gt;\.json and `fixtures\/<task>\/`/);
  assert.match(rendered, /<details><summary>Authored HTML<\/summary>Kept intact\.<\/details>/);
...
test('placeholder escaping is narrow around Markdown code spans and authored HTML', () => {
  assert.equal(escapePlaceholderTags('Use <path>, `<path>`, and <kbd>Enter</kbd>.'), 'Use &lt;path&gt;, `<path>`, and <kbd>Enter</kbd>.');
  assert.equal(escapePlaceholderTags('Use ``a `<path>` b`` then <name>.'), 'Use ``a `<path>` b`` then &lt;name&gt;.');
  assert.equal(escapePlaceholderTags('An unmatched ` is prose with <name>.'), 'An unmatched ` is prose with &lt;name&gt;.');
});
  ...
  for (const repository of ['website', ...repositories.map(([id]) => id)]) {
    ...
    assert.deepEqual(project, [
      ...
      {type: 'link', label: 'All technical docs', href: '/docs/'},
      {
        ...
        label,
        collapsed: false,
        link: {type: 'doc', id: `${repository}/index`},
        items: [{type: 'autogenerated', dirName: repository}],
      },
    ]);
  }
});
...
test('discovery and profile views compose the shared presentation primitives', async () => {
  ...
  assert.doesNotMatch(files[2], /dangerouslySetInnerHTML/);
});
...
test('a family emptied by quarantine is counted over the published roster and drops out of the sidebar', () => {
  const published = [['agentic-principles', 'Foundation'], ['harness', 'Build'], ['agent-platform', 'Services']];
  const manifests = [{repository: {id: 'website'}, surfaces: [{source: {navigation: {label: 'Start', order: 0, sidebar: 'flat'}}}]}]
    .concat(published.map(([id, group], order) => ({repository: {id}, surfaces: [{source: {navigation: {label: id, group, order, sidebar: 'autogenerated'}}}]})));
  assert.throws(() => assertDocumentationFamilyDistribution(manifests), /Products documentation family has no public repositories/);
  assert.deepEqual(assertDocumentationFamilyDistribution(manifests, {quarantined: ['devcenter']}), {Foundation: 1, Build: 1, Services: 1, Products: 0});
  const withoutServices = manifests.filter((manifest) => manifest.repository.id !== 'agent-platform');
  assert.throws(
    () => assertDocumentationFamilyDistribution(withoutServices, {quarantined: ['devcenter']}),
    /2 documentation families have no public repositories, more than the 1 quarantined source could have emptied/,
  );
  const registry = {surfaces: [
    {name: 'Start', kind: 'front-door', repository: {id: 'website'}},
    ...published.map(([id]) => ({name: id, repository: {id}})),
  ]};
  const document = JSON.parse(renderSidebars(registry, manifests).replace(/^module\.exports = /, '').replace(/;\n$/, ''));
  const families = document.docs.slice(2, -1);
  assert.deepEqual(families.map((family) => family.label), documentationFamilies.filter((family) => family.id !== 'Products').map((family) => family.label));
  assert.equal(document.project_devcenter, undefined);
});

test('each family start repository is a published member of its family unless it is quarantined', () => {
  const member = (id, group) => ({repository: {id}, surfaces: [{source: {navigation: {label: id, group, order: 0, sidebar: 'autogenerated'}}}]});
  const manifests = [member('agentic-principles', 'Foundation'), member('harness', 'Build'), member('identity', 'Services'), member('devcenter', 'Products')];
  assert.throws(() => assertDocumentationFamilyDistribution(manifests), /Services start agent-platform is not a published member of Services/);
  assert.deepEqual(assertDocumentationFamilyDistribution(manifests, {quarantined: ['agent-platform']}), {Foundation: 1, Build: 1, Services: 1, Products: 1});
  const misplaced = [...manifests, member('agent-platform', 'Build')];
  assert.throws(() => assertDocumentationFamilyDistribution(misplaced), /Services start agent-platform is not a published member of Services/);
});
